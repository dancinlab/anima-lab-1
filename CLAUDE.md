...
```
모든 엔진/아키텍처는 아래 5개 조건을 반드시 통과해야 함.
bench_v2.py --verify 로 검증. 1개라도 실패 시 배포 금지.

관문은 음성 대조군 6종(시체·난수·복제·뒤섞기·HEAP·입력무관)으로 검증된다.
여섯 전부 0/5 여야 하고, 실제 엔진은 최소 하나를 통과해야 한다.
tests/test_gate_controls.py 가 이 성질을 강제한다 (대조군 6 × 조건 5
+ 양성 가드 = 31 passed). 축을 무력화하면 시체가 ZERO_INPUT 을 통과해
테스트가 즉시 실패한다 — 잡을 수 있는 실패가 실재함을 확인했다.
  ...
  ※ 위 다섯은 모두 3축 선결조건을 함께 만족해야 통과:
     통합(한 세포를 흔들면 나머지가 움직임) · 시간정체성(내 지금이 남의
     직전보다 내 직전에서 이어짐) · 변화(상태가 실제로 움직임) ·
     응답(다른 입력에 궤적이 갈라짐). 각 기준선은 상수가 아니라 그 집단
     자신의 귀무(붕괴형·순열형)에서 측정된다.

폐기된 조건 2개 (bench_v2.py `_RETIRED_TESTS`, 측정 근거 동봉):

  SPONTANEOUS_SPEECH — 이름과 음의 상관. 뒤섞기 대조군 6.8 vs 실제 엔진 1.0.
     붕괴한 집단만 기준선 5에 도달(코사인 1.0000에서 8~18회). 14개 경로
     폐쇄, 재정의 4회 시도 전부 대조군에게 뚫림.

  HIVEMIND — 이 세션 전까지 한 번도 실행된 적 없음(11개 엔진 전부 0.3~1.6초
     크래시). 돌게 만든 뒤 3회 반복 + 미연결 대조군으로 재니 연결 효과가
     −4%~+1%, 요구치는 +10%.

  근거: docs/consciousness-gate-audit.md
